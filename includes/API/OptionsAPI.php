...
class OptionsAPI {
	...
	public function get_option( WP_REST_Request $request ) {
		...
		if ( ! empty( $key ) ) {
			if ( $options->has( $key ) ) {
				...
				return new WP_Error( 'option_error', __( 'Invalid or expired option name.', 'music-slider' ) );
			}
		} else {
			$result = $options->get();
		}
	...
	public function set_option( WP_REST_Request $request ) {
		...
		if ( ! empty( $settings ) && is_array( $settings ) ) {
			...
			return new WP_Error( 'settings_error', __( 'No settings provided.', 'music-slider' ) );
		}

		return new WP_REST_Response(
			array(
				'message' => __( 'Settings updated.', 'music-slider' ),
			),
			200
		);
	}
	...
	public function delete_option( WP_REST_Request $request ) {
		...
		if ( ! empty( $key ) ) {
			...
			return new WP_Error( 'option_error', __( 'No option key is provided.', 'music-slider' ) );
		}

		return new WP_REST_Response(
			array(
				'message' => __( 'Setting deleted.', 'music-slider' ),
			),
			200
		);
	}
