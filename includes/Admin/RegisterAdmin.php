...
class RegisterAdmin {
	...
	public function register_admin_dashboard() {
		$primary_slug = BPLMS_SLUG;

		$dashboard_page_suffix = add_menu_page(
			_x( 'Music_Slider Dashboard', 'Page title', 'music-slider' ),
			_x( 'Music_Slider', 'Menu title', 'music-slider' ),
			'manage_options',
			$primary_slug,
			array( $this, 'plugin_dashboard_page' ),
			BPLMS_URL . '/assets/img/icon.svg',
			30
		);
		...
		add_submenu_page( $primary_slug, 'Music_Slider Dashboard', 'Dashboard', 'manage_options', $primary_slug . '#/dashboard', '__return_null' );
		...
		add_submenu_page( $primary_slug, 'Music_Slider Dashboard', 'Getting Started', 'manage_options', $primary_slug . '#/getting-started', '__return_null' );

		// Register changelog submenu nav item.
		add_submenu_page( $primary_slug, 'Music_Slider Dashboard', 'Changelog', 'manage_options', $primary_slug . '#/changelog', '__return_null' );

		// Register settings submenu nav item.
		add_submenu_page( $primary_slug, 'Music_Slider Dashboard', 'Settings', 'manage_options', $primary_slug . '#/settings', '__return_null' );
	}
	...
	public function initial_dashboard_state() {
		return array(
			'apiRoute'     => BPLMS_SLUG . '/v1',
			'assetsURL'    => BPLMS_URL . '/assets',
			// You can also replace this changelog URL to something else so that it loads from one source and stays up-to-date always.
			'changelogURL' => BPLMS_URL . '/changelog.json?ver=' . filemtime( BPLMS_DIR . '/changelog.json' ),
			'version'      => BPLMS_VERSION,
		);
	}
	...
	public function plugin_dashboard_page() {
		?>
			<div id="music-slider-dashboard-root"></div>
		<?php
	}
	...
	public function setup_admin_bar_menu() {
		...
		$action_key = BPLMS_SLUG . '_flush_rules';
	...
	public function register_admin_bar_menu( WP_Admin_Bar $admin_bar ) {
		...
		$admin_bar->add_menu(
			array(
				'id'     => $parent_menu_id,
				'parent' => null,
				'title'  => __( 'Music_Slider Tools', 'music-slider' ),
		...
		$admin_bar->add_menu(
			array(
				...
				'title'  => __( 'Flush Permalinks', 'music-slider' ),
