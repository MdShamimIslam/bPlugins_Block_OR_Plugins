...
1. Find an appropriate [content-slider](https://wordpress.org/plugins/) according to the solution of the plugin.
...
3. If your block is not part of `b-blocks`, then the text domain should be your `content-slider`.
4. Apply 1st and 2nd list data to this template with case sensitivity: uppercase for uppercase, lowercase for lowercase, title case for title case, and camel case for camel case. (for the block name try different way to search (content-slider, contentSlider, ContentSlider, Content Slider, block name))
5. Complete the `readme.txt` file.
6. The main element is `.wp-block-b-blocks-{content-slider}`, and its immediate child is `.bBlocksContentSlider`. Do not apply width or columns for the main element.
