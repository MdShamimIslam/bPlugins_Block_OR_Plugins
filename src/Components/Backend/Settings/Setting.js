...
import { PanelBody, PanelRow, TextControl, __experimentalUnitControl as UnitControl, Button, Dashicon, SelectControl } from '@wordpress/components';
...
const Setting = ({ attributes, setAttributes, setActiveIndex }) => {
	...
	return <>
		<InspectorControls>
			...
			{songSl === 'default' && <PanelBody className='bPlPanelBody' title={__('Player Settings', 'mp3player-block')}>
		...
		<BlockControls>
			<AlignmentToolbar label="Player Alignment" value={alignment} onChange={val => setAttributes({ alignment: val })} describedBy={__('Player Alignment')} alignmentControls={[
