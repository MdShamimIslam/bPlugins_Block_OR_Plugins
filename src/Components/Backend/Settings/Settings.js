		...
		<InspectorControls>
			...
			<TabPanel className='bPlTabPanel' activeClass='activeTab' tabs={generalStyleTabs} onSelect={tabController}>{tab => <>
				{'general' === tab.name && <>
					...
					<PanelBody className='bPlPanelBody' title={__('Component Settings', 'textdomain')} initialOpen={false}>
					...
					<PanelBody className='bPlPanelBody' title={__('Custom Components', 'textdomain')} initialOpen={false}>
						...
						<IconControl className='mt20' icon={icon} onChange={val => setAttributes({ icon: val })} defaults={{ class: 'fa-brands fa-wordpress' }} version={6} />
				...
				{'style' === tab.name && <>
					<PanelBody className='bPlPanelBody' title={__('Custom Style', 'textdomain')}>
						...
						<Typography className='mt20' label={__('Typography:', 'textdomain')} value={typography} onChange={val => setAttributes({ typography: val })} defaults={{ fontSize: 25 }} />
						...
						<br />
						<br />
						<BoxControl label={__('Padding:', 'textdomain')} values={padding} onChange={val => setAttributes({ padding: val })} />
						...
						<br />
						<br />
						<BoxControl label={__('Margin:', 'textdomain')} values={margin} onChange={val => setAttributes({ margin: val })} />

						<BorderControl label={__('Border:', 'textdomain')} value={border} onChange={val => setAttributes({ border: val })} defaults={{ radius: '5px' }} />

						<MultiShadowControl label={__('Shadow:', 'textdomain')} value={shadow} onChange={val => setAttributes({ shadow: val })} />
					</PanelBody>
				</>}
			</>}</TabPanel>
		</InspectorControls>
